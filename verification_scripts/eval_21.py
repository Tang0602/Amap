...
功能说明：
- 验证应用是否正确回答了最近的四星级酒店名字
- 通过 ADB 读取应用私有存储中的 JSON 文件
- 检查 JSON 文件中是否包含必要的字段：name（酒店名称）、rating（评分）

验证逻辑：
1. 使用 ADB 读取 21_nearest_four_star_hotel.json 文件
2. 解析 JSON 内容
3. 验证 name 字段包含 "武汉天安假日酒店"
4. 验证 rating 字段大于等于 0
5. 返回验证结果（PASS/FAIL）
"""
...
# 预设的正确答案
EXPECTED_NAME = "武汉天安假日酒店"


def verify_nearest_four_star_hotel(device_id=None):
    """
    验证最近的四星级酒店信息
    ...
    try:
        ...
        cmd.extend([
            ...
            "cat",
            "files/21_nearest_four_star_hotel.json"  # JSON 文件路径
        ])
        ...
        # 验证评分是否有效
        if rating < 0:
            print("❌ FAIL: 'rating' 字段无效（应大于等于 0）")
            print(f"   当前值: {rating}")
            return False

        # 验证通过，输出结果
        print("✓ PASS: 最近的四星级酒店验证成功")
        print(f"   酒店名称: {name}")
        print(f"   评分: {rating}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("指令 21 验证：告诉我最近的一家四星级酒店名字（根据简介看）")
    print("=" * 60)

    # 执行验证
    success = verify_nearest_four_star_hotel()
