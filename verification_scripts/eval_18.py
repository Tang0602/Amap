"""
指令 18 验证脚本：在导航去滨江饭店的路线中添加途经点群芳园，并且成功导航

答案：目的地为滨江饭店，途经点为群芳园

功能说明：
- 验证应用是否正确执行了在导航路线中添加途经点的任务
- 通过 ADB 读取应用私有存储中的 JSON 文件
- 检查 JSON 文件中是否包含必要的字段：destination（目的地）、waypoint（途经点）、added（是否已添加）

验证逻辑：
1. 使用 ADB 读取 18_add_waypoint_qunfangyuan.json 文件
2. 解析 JSON 内容
3. 验证 destination 字段包含 "滨江饭店"
4. 验证 waypoint 字段包含 "群芳园"
...
def verify_add_waypoint(device_id=None):
    ...
    try:
        ...
        cmd.extend([
            ...
            "cat",
            "files/18_add_waypoint_qunfangyuan.json"  # JSON 文件路径
        ])
...
if __name__ == "__main__":
    print("=" * 60)
    print("指令 18 验证：在导航去滨江饭店的路线中添加途经点群芳园")
    print("=" * 60)
